    ...
    internal class Tristana : Champion
    {
        ...
        public override void Orbwalking_BeforeAttack(Orbwalking.BeforeAttackEventArgs args)
        {
            ...
            if (t != null && ComboActive && args.Unit.IsMe)
            {
                ...
                if (useQ && (t.HasBuff("TristanaECharge") || t.HasBuff("TristanaEChargeSound") || Player.HealthPercent < 70 || t.HealthPercent < 60))
                    Q.Cast();
        ...
        public override void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (useRB && Player.HealthPercent < 40)
            {
                var enemy =
                    HeroManager.Enemies.OrderBy(target => target.TotalAttackDamage)
                        .LastOrDefault(targ => Player.Distance3D(targ) <= 275 && targ.IsMelee());

...
namespace GodlikeSkillz
{
    internal class Lucian : Champion
    {
                ...
                return (from vMinion in vMinions.Where(vMinion => vMinion.IsValidTarget(Q.Range))
                    let endPoint =
                        vMinion.ServerPosition.To2D()
                            ...
                            .To3D()
                    where
                        vMinion.Distance3D(vTarget) <= vTarget.Distance3D(ObjectManager.Player) &&
                        Intersection(
        ...
        public override void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (_pCast && ObjectManager.Player.HasBuff("lucianpassivebuff"))
            {
                _pCast = false;
                ls = true;
            }
            if (ls && !_pCast && !ObjectManager.Player.HasBuff("lucianpassivebuff"))
            {
                ls = false;
            }
            ...
            if (Q.IsReady() && GetValue<KeyBind>("UseQExtendedTH").Active && ToggleActive)
            {
                ...
                {
                    if (ObjectManager.Player.Distance3D(t) > Q.Range)
                    {

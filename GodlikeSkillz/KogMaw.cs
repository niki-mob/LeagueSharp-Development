    ...
    internal class Kogmaw : Champion
    {
        ...
        public override void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (useW && W.IsReady())
            {
                ...
                if (targ != null)
                {
                    if (targ.Name == Orbwalker.GetTarget().Name && (Player.HealthPercent < 70 || targ.HealthPercent < 70))
                    {
                        W.CastOnUnit(Player);
                    }
                    else
            ...
            if (useR && R.IsReady() && UltimateBuffStacks < rLim
                && (Player.ManaPercent >= GetValue<Slider>("RlimM").Value || UltimateBuffStacks <= 1))
            {
                var t = Orbwalker.GetTarget() as Obj_AI_Hero ??
                        TargetSelector.GetTarget(R.Range, TargetSelector.DamageType.Magical);
                if (t != null && (ObjectManager.Player.Distance3D(t) > Orbwalking.GetRealAutoAttackRange(t) || (AttackReadiness > 0.2 && AttackReadiness < 0.9)))
                    R.Cast(t, false, true);
            }
            ...
            if (useE && E.IsReady() && Player.ManaPercent >= GetValue<Slider>("RlimM").Value)
            {

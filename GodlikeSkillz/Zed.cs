...
namespace GodlikeSkillz
{
    internal class Zed : Champion
    {
        ...
        {
            return HeroManager.Enemies.FirstOrDefault(enemyVisible => enemyVisible.IsValidTarget() && enemyVisible.HasBuff("zedulttargetmark"));
        }
        ...
        static Obj_AI_Hero GetEnemy
        {
            get
            {
                ...
                foreach (var tar in HeroManager.Enemies.Where(tar => tar.IsValidTarget() && (tar.Distance3D(Player) < Q.Range || (WShadowPos != new Vector3() && tar.Distance(WShadowPos) < Q.Range) || (RShadow != null && tar.Distance3D(RShadow) < Q.Range))).Where(tar => targ[0] == null || PhysHealth(tar) < PhysHealth(targ[0])))
                {                
                    targ[0] = tar;
                }

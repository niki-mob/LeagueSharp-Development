    ...
    internal class Program
    {
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (CClass.Orbwalk)
            {
                ...
                CClass.HarassActive = CClass.Config.Item("Farm").GetValue<KeyBind>().Active &&
                                      (!CClass.UsesMana || ObjectManager.Player.ManaPercent >= vHarassManaPer);

                CClass.ToggleActive = !CClass.UsesMana || ObjectManager.Player.ManaPercent >= vHarassManaPer;
                ...
                CClass.LaneClearActive = CClass.Config.Item("LaneClear").GetValue<KeyBind>().Active &
                                         (!CClass.UsesMana || ObjectManager.Player.ManaPercent >= vLaneClearManaPer);
            }
            ...
            if (botrk)
            {
                    ...
                    target.ServerPosition.Distance(ObjectManager.Player.ServerPosition) < 450)
                {
                    ...
                    if (hasBotrk || hasCutGlass)
                    {
                        ...
                        if (ObjectManager.Player.HealthPercent < 60 || target.HealthPercent < 70)
            ...
            if (ghostblade && target.IsValidTarget() &&
                Orbwalking.InAutoAttackRange(target) && (ObjectManager.Player.HealthPercent < 60 || target.HealthPercent < 70))
        ...
        public static void UseSummoners()
        {
            ...
            if (Config.Item("SUMIGNITEENABLE").GetValue<bool>())
            {
                ...
                {
                    if (ObjectManager.Player.Distance3D(t) < 650 &&
        ...
        private static void OrbwalkingOnAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            {
                if (target != null &&
                    ((target.Type == GameObjectType.obj_AI_Hero && !ObjectManager.Player.HasBuff("Muramana") ||
                     (target.Type != GameObjectType.obj_AI_Hero && ObjectManager.Player.HasBuff("Muramana")))))
                {

...
namespace GodlikeSkillz
{
    internal class Kennen : Champion
    {
        ...
        public Kennen()
        {
            Utils.PrintMessage("Kennen loaded");
        ...
        public override void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (useW)
            {
                ...
                {
                    if (StormMarkStacks(enemy) > 0 && (StormMarkStacks(enemy) == 2 || StormMarkTimeLeft(enemy) < 0.5 || (Player.Distance3D(enemy) > 800 && Prediction.GetPrediction(enemy, 1).UnitPosition.Distance(Prediction.GetPrediction(Player, 1).UnitPosition) > 900) || (GetValue<bool>("UseWM") && enemy.Health < GetSurgeDamage(enemy))))
                    {
                        W.Cast();
                    }
                }
